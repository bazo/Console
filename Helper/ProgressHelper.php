...
class ProgressHelper extends Helper
{
	...
	const FORMAT_VERBOSE_MEMORY = ' %current%/%max% [%bar%] %percent%% Elapsed: %elapsed% Memory: %memory% Peak: %peak%';
	 ...
	 * @var array
	 */
	private $memoryFormats = array(
		array(1024, 'B'),
		array(1048576, 'KiB', 1024),
		array(1073741824, 'MiB', 1048576),
		array(1024, 'GiB', 1073741824),
	);
	...
	private function generate($finish = false)
	{
		...
		if (isset($this->formatVars['elapsed'])) {
			...
			$vars['elapsed'] = str_pad($this->fullTime($elapsed), $this->widths['elapsed'], ' ', STR_PAD_LEFT);
		}
		...
		if (isset($this->formatVars['memory'])) {
			$memory = memory_get_usage($realUsage = TRUE);
			$vars['memory'] = str_pad($this->humaneMemory($memory), $this->widths['percent'], ' ', STR_PAD_LEFT);
		}

		if (isset($this->formatVars['peak'])) {
			$memory = memory_get_peak_usage($realUsage = TRUE);
			$vars['peak'] = str_pad($this->humaneMemory($memory), $this->widths['percent'], ' ', STR_PAD_LEFT);
		}
		return $vars;
	}
	...
	private function fullTime($secs)
	{
	...
	private function humaneMemory($memory)
	{
		$text = '';
		foreach ($this->memoryFormats as $format) {
			if ($memory < $format[0]) {
				if (count($format) == 2) {
					$text = $format[1];
					break;
				} else {
					$text = ceil($memory / $format[2]) . ' ' . $format[1];
					break;
				}
			}
		}
